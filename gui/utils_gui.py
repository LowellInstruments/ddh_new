...
from PyQt5.QtWidgets import QTabWidget, QDesktopWidget, QWidget, QMessageBox, QListWidgetItem, QTableWidgetItem, \
    QHeaderView
from gpiozero import Button
from context import ctx
from db.db_his import DBHis
from settings.version import VER_SW
from threads.th_life import ButtonPressEvent
from threads.utils import json_get_ship_name, linux_rpi
...
def setup_view(my_win, j):
    ...
    a.img_time.setPixmap(QPixmap('gui/res/img_datetime_color.png'))
    a.img_sync.setPixmap(QPixmap('gui/res/img_sync_color.png'))
    a.img_boat.setPixmap(QPixmap('gui/res/img_boatname_color.png'))
    a.img_ble.setPixmap(QPixmap('gui/res/img_blue_color.png'))
    a.img_latnlon.setPixmap(QPixmap('gui/res/img_gps_dis.png'))
    a.img_output.setPixmap(QPixmap('gui/res/img_wait_color.png'))
    a.img_cloud.setPixmap(QPixmap('gui/res/img_upload_color.png'))
...
def setup_buttons_gui(my_app):
    ...
    a.img_time.mousePressEvent = a._click_icon_clock
    a.img_ble.mousePressEvent = a._click_icon_ble
    a.img_latnlon.mousePressEvent = a._click_icon_gps
    a.img_sync.mousePressEvent = a._click_icon_net
    a.img_cloud.mousePressEvent = a._click_icon_cloud
    a.img_output.mousePressEvent = a._click_icon_output
...
def update_gps_icon(my_app, did_ok, lat, lon):
    ...
    if not did_ok:
        ...
        a.img_latnlon.setPixmap(QPixmap(img))
        return
