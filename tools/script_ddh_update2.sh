...
if ! cp conf/run_ddh.sh $FOL; then echo "error -> run_ddh.sh"; exit 1; fi
if ! cp conf/ddh.json $FST; then echo "error -> ddh.json"; exit 1; fi
if ! cp conf/_macs_to_sn.yml $FST; then echo "error -> _macs file"; exit 1; fi
rm -rf conf || true
