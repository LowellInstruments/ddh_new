Recall python project source folder is somehow repeated:
...
For sphinx to find .RST files, edit 'docs/source/conf.py'
    ...
    import sys
    _path = '../../'
    # _my_path leaves us inside <module_root> explained above
    # NOT inside <module_name>
    sys.path.insert(0, os.path.abspath(_path)

For sphinx-autodoc to use Google docstrings, also in conf.py, add:
...
For sphinx-autodoc to create .RST files from .PY files:
    ...
    # great! it is smart enough to grab main.py from its location
...
For sphinx to manually build .HTML from .RST files:
    # from .rst files generate .html files
    $ cd docs
    $ make html
