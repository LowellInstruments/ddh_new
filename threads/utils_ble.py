...
def emit_scan_post(sig, sr):
    if sig:
        sig.ble_scan_post.emit(sr)
...
def emit_error_gui(sig, e):
    if sig:
        sig.ble_status.emit(e)
...
def emit_logger_post(sig, got):
    if sig:
        sig.ble_logger_post.emit(got)
...
def emit_file_post(sig, x, speed):
    if sig:
        sig.ble_file_post.emit(x, speed)
...
# rv ok -> log / not -> raise our exception
# def _ok_or_die(c, rv, sig):
#     txt = 'BLE: {}{}'.format(c, rv)
#     if rv and sig:
#         emit_status(sig, txt)
#         return
#     raise AppBLEException(txt)
...
def _logger_sws(lc, sig, g):
    ...
    _ok_or_die([b'SWS', b'00'], rv, sig)


def _logger_time_check(lc, sig=None, g=None):
    # command: GTM
    rv = lc.get_time()
    _show(rv, sig)
...
def _logger_get_files(lc, sig, folder, files):
    ...
    got = 0
    b_left = b_total
    s = 'BLE: {} has {} files we don\'t'.format(mac, num_to_get)
    emit_status(sig, s)
    ...
    for name, size in name_n_size.items():
        ...
        if exists_file(name, size, folder):
            ...
            emit_file_post(sig, x, speed)

    emit_logger_post(sig, got)
...
def _logger_bye(mac, sig):
    s = 'BLE: bye {}'.format(mac)
    emit_status(sig, s)


def _logger_see_you(mac, sig):
    s = 'BLE: see_you {}'.format(mac)
    emit_status(sig, s)


def _logger_not_bye(mac, sig):
    s = 'BLE: what {}'.format(mac)
    emit_error(sig, s)
    ...
    try:
        with LoggerControllerBLE(mac, hci_if) as lc:
            ...
            if got_all:
                ...
                _logger_see_you(mac, sig)
                return False
    except AppBLEException as ex:
        emit_error(sig, 'error: {}'.format(ex))
        _logger_not_bye(mac, sig)
        return False
