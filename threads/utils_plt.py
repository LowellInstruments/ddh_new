...
def emit_result(sig, r):
    if sig:
        sig.plt_result.emit(r)
...
def _cache_or_process(sig, folder, ts, metric, sd):
    ...
    t, y_avg = _slice_n_avg(x, y, sd[ts])
